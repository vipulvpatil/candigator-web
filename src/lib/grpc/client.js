...
const grpcServiceClient = () => {
  if (!global.grpcServiceClient || global.refreshGrpcClient) {
    ...
    global.grpcServiceClient = {
      ...
      getUserData: promisify(client.GetUserData).bind(client),
      ...
      getFileUpload: promisify(client.GetFileUpload).bind(client),
      deleteFileUpload: promisify(client.DeleteFileUpload).bind(client),
      getCandidates: promisify(client.GetCandidates).bind(client),
      getCandidate: promisify(client.GetCandidate).bind(client),
      updateCandidate: promisify(client.UpdateCandidate).bind(client)
    }
  }
